...
function detectTechStack(files: ScannedFile[]): string[] {
    ...
    if (fileNames.includes('package.json')) {
        ...
        if (pkg) {
            try {
                ...
                if (allDeps['nuxt']) stack.push('Nuxt.js');
                if (allDeps['svelte']) stack.push('Svelte');
                if (allDeps['@sveltejs/kit']) stack.push('SvelteKit');
    ...
    if (fileNames.includes('build.gradle') || fileNames.includes('pom.xml')) stack.push('Java/JVM');
    if (allPaths.some(p => p.endsWith('.dart'))) stack.push('Dart/Flutter');
