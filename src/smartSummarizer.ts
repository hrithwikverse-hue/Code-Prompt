...
import { getGitBranch, getRecentCommits, isGitRepo } from './gitUtils';
import {
    HIGH_PRIORITY_FILENAMES,
    FILENAME_TECH_SIGNALS,
    NPM_DEP_SIGNALS,
    PYTHON_DEP_SIGNALS,
    EMBEDDED_EXTENSIONS,
} from './fileRegistry';
...
export function smartExport(
    ...
    if (isGitRepo(workspacePath)) {
        ...
        const commits = getRecentCommits(workspacePath, 5);
...
function scoreFile(file: ScannedFile): FilePriority {
    let score = 0;
    ...
    const hpBonus = HIGH_PRIORITY_FILENAMES.get(name);
...
function detectTechStack(files: ScannedFile[]): string[] {
    ...
    for (const [signal, labels] of FILENAME_TECH_SIGNALS) {
    ...
    const pkg = files.find(f =>
        f.relativePath === 'package.json' || f.relativePath.endsWith('/package.json')
    );
    if (pkg) {
        ...
        try {
            ...
            for (const [dep, label] of NPM_DEP_SIGNALS) {
    ...
    if (fileNameSet.has('bun.lockb') || fileNameSet.has('bunfig.toml')) stack.add('Bun');
    if (fileNameSet.has('deno.json') || fileNameSet.has('deno.jsonc') || fileNameSet.has('deno.lock')) stack.add('Deno');

    // --- Python requirements.txt dependency signals ---
    const reqTxt = files.find(f =>
        f.relativePath === 'requirements.txt' || f.relativePath.endsWith('/requirements.txt')
    );
    if (reqTxt) {
        stack.add('Python');
        const lower = reqTxt.content.toLowerCase();
        for (const [dep, label] of PYTHON_DEP_SIGNALS) {
            if (lower.includes(dep)) stack.add(label);
        }
    }

    // --- pyproject.toml dependency signals ---
    const pyproject = files.find(f =>
    ...
    // --- Rust ---
    if (fileNameSet.has('cargo.toml')) stack.add('Rust');

    // --- Go ---
    if (fileNameSet.has('go.mod')) stack.add('Go');

    // --- Ruby ---
    if (fileNameSet.has('gemfile')) stack.add('Ruby');

    // --- Java/JVM ---
    if (fileNameSet.has('build.gradle') || fileNameSet.has('pom.xml')) stack.add('Java/JVM');
    ...
    // --- Swift / iOS / macOS ---
    if (allPaths.some(p => p.endsWith('.swift'))) stack.add('Swift / iOS / macOS');

    // --- Kotlin / Android ---
    if (allPaths.some(p => p.endsWith('.kt') || p.endsWith('.kts'))) stack.add('Kotlin / Android');

    // --- Zig ---
    if (allPaths.some(p => p.endsWith('.zig'))) stack.add('Zig');
    ...
    if (hasIno) stack.add('Arduino / Embedded C++');
    else if (hasCppFiles) stack.add('C++');
    else if (hasCFiles) stack.add('C');
