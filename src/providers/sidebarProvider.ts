...
import { generateMarkdown, safeMdPath } from '../extension';

export class SidebarProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'aiContext.sidebar';
            ...
            switch (message.command) {
                ...
                case 'copyMd': {
                    ...
                    if (wp && message.mdFile) {
                        const md = await generateMarkdown(message.mdFile, wp);
                        if (md) {
                            if (message.mdFile !== 'CHUNKED_CONTEXT') {
                                const dir = path.join(wp, '.code prompt');
                                if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
                                const filePath = safeMdPath(dir, message.mdFile);
                                if (filePath) fs.writeFileSync(filePath, md, 'utf-8');
                            }
    ...
    private _getHtml(): string {
        ...
        const aiIcons = (mdFile: string) => AI.map(a =>
            `<span class="ai-icon" data-tip="Copy & open ${a.name}" onclick="copyAndOpen(event,'${a.url}','${mdFile}')"><img src="${a.favicon}" alt="${a.name}"></span>`
        ).join('') + `<span class="ai-icon copy-icon" data-tip="Copy MD to clipboard" onclick="copyMd(event,'${mdFile}')"><svg viewBox="0 0 16 16"><path d="M4 4h5l3 3v6H4V4zm1 1v7h6V8H8V5H5zm4 0v2h2l-2-2zM2 2h6v1H3v8H2V2z"/></svg></span>`;

        const btn = (cmd: string, mdFile: string, name: string, desc: string, tip: string, secondary = false, showAi = true) => `
...
<body>
    ...
    ${btn('exportFull', 'PROJECT_CONTEXT',
        'Full Project', 'PROJECT_CONTEXT.md &middot; Every file in your project',
        'Exports every file into PROJECT_CONTEXT.md. Best for full AI visibility of your codebase.'
    )}

    ${btn('exportSmart', 'SMART_CONTEXT',
        'Smart Export', 'SMART_CONTEXT.md &middot; Only the files that matter most',
        'AI-ranked export of key files only. Ideal for ChatGPT Free — stays under token limits.'
    )}

    ${btn('exportSrc', 'SRC_CONTEXT',
        'Source Only', 'SRC_CONTEXT.md &middot; Just your source code folder',
        'Exports only src/, lib/, app/ etc. Skips config, docs and other noise.'
    )}

    ${btn('exportCurrentFile', 'CURRENT_FILE_CONTEXT',
        'Current File', 'CURRENT_FILE_CONTEXT.md &middot; Active editor tab only',
        'Exports only the file currently open in the editor. Great for single-file AI review.'
    )}

    ${btn('exportChunked', 'CHUNKED_CONTEXT',
        'Chunked Export', '.code prompt/chunked/context_part_N.md &middot; Split large projects',
        'Splits your project into paste-sized .md chunks. Use for large codebases.'
    )}

    <hr class="divider">

    ${btn('configure', '',
        'Settings', 'Configure export preferences',
        'Opens VS Code settings for Code Prompt. Set ignored folders, file size limits, output location and more.',
        false, false
    )}
